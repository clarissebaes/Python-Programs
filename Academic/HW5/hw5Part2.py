...
print(prob)
simnum = int(input('Enter the number of simulations to run => '))
print(simnum)
...
def run_one_simulation(position,bounds,prob,grid):
...
count_grid = []
for i in range(mrow):
    count_grid.append([0]*ncol)

for j in range(simnum):
    i = 0        
    while i < 250:
        output = run_one_simulation(position,bounds,prob,count_grid)
        #l = input('==> ')
        position = output[0]
...
def printgrid(gridcount):
    for i in range(mrow):
        outgrid = str(count_grid[i]).replace('[', '').replace(']','').replace(',','')
        print(outgrid)

printgrid(count_grid)

print('Total pokemon caught is {:}'.format(pokenumtotal))
